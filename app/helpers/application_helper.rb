module ApplicationHelper
  ...
  def play_button(testable, text=nil)
    ...
    if project.creator.has_minutes_available?
      html = link_to play_glyph(text: text), "javascript:void(0)", "data-target" => "#selectBrowers_#{testable.id}", "data-toggle" => "modal", class: "btn btn-primary btn-sm"
    else
      html = link_to play_glyph(text: text), edit_user_registration_path(anchor: "change-plan"), class: "btn btn-primary btn-sm"
    end
    html.html_safe
  end
